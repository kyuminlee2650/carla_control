...
def main():
    ...
    parser.add_argument("--planner-hz", type=float, default=10.0, help="mock planner replanning rate (Hz)")
    parser.add_argument("--planner-horizon-s", type=float, default=3,
                         help="mock planner lookahead horizon (s); also sets point spacing (shorter = denser)")
    parser.add_argument("--planner-num-points", type=int, default=6, help="mock planner waypoint count")
    ...
    last_idx = 0
    true_last_idx = 0  # tracks position against the fixed global path, independent of the local plan

    # Mock "planner": replans a short ego-relative trajectory off the full desired path every
    # planner_period_ticks control ticks. The controller tracks this short plan (plan_path_*),
    # not the full desired path (path_*) -- path_* stays around only for the goal check / background plot.
    planner = MockPlanner(path_x, path_y, path_yaw, horizon_s=args.planner_horizon_s, num_points=args.planner_num_points)
    planner_period_ticks = max(1, round(1.0 / (args.planner_hz * args.dt)))
    plan_path_x, plan_path_y, plan_path_yaw = path_x, path_y, path_yaw

    bev = None if args.no_live_view else BevView(path_x, path_y)

    hist = {"t": [], "x": [], "y": [], "speed": [], "steer": [], "steer_deg": [], "throttle": [], "brake": [],
            "cte": [], "true_cte": [], "yaw": [], "path_yaw": [], "heading_error": []}
    ...
    try:
        ...
        for i in range(steps):
            ...
            if i % planner_period_ticks == 0:
                ego_transform = vehicle.get_transform()
                ego_x, ego_y = ego_transform.location.x, ego_transform.location.y
                ego_yaw = math.radians(ego_transform.rotation.yaw)
                local_wps = planner.plan(ego_x, ego_y, ego_yaw, speed_ms(vehicle))
                world_wps = local_to_world(local_wps, ego_x, ego_y, ego_yaw)
                plan_path_x = [p[0] for p in world_wps]
                plan_path_y = [p[1] for p in world_wps]
                plan_path_yaw = []
                for j in range(len(world_wps)):
                    if j < len(world_wps) - 1:
                        dxp, dyp = world_wps[j + 1][0] - world_wps[j][0], world_wps[j + 1][1] - world_wps[j][1]
                    else:
                        dxp, dyp = world_wps[j][0] - world_wps[j - 1][0], world_wps[j][1] - world_wps[j - 1][1]
                    plan_path_yaw.append(math.atan2(dyp, dxp))
                last_idx = 0  # plan_path always starts at the vehicle's current position (index 0)

                wp_str = " ".join(f"({x:.1f},{y:.1f})" for x, y in zip(plan_path_x, plan_path_y))
                print(f"t={i*args.dt:5.1f}s  ego=({ego_x:.1f},{ego_y:.1f})  plan_wps: {wp_str}")
                if bev is not None:
                    bev.update(ego_x, ego_y, ego_yaw, plan_path_x, plan_path_y)

            target_idx, delta, (fx, fy), cte, yaw, theta_e = stanley_control(
                vehicle, wheelbase, plan_path_x, plan_path_y, plan_path_yaw, last_idx
            )
            last_idx = target_idx
            true_last_idx, true_cte = lateral_error(fx, fy, yaw, path_x, path_y, true_last_idx)
            ...
            control_value = cliping(speed_filter.step(pid.step(speed_error)),1,-1)
            ...
            hist["cte"].append(cte)
            hist["true_cte"].append(true_cte)
            hist["yaw"].append(math.degrees(yaw_unwrapper.step(yaw)))
            hist["path_yaw"].append(math.degrees(rh_unwrapper.step(plan_path_yaw[target_idx])))
            hist["heading_error"].append(math.degrees(theta_e))

            since_replan = i % planner_period_ticks
            print(f"t={t:5.1f}s  since_replan={since_replan}  target_idx={target_idx}/{len(plan_path_x)-1}  "
                  f"speed={current_speed_kmh:5.1f} km/h  steer={steer:+.2f}  cte={cte:+.2f} m")

            dist_to_goal = math.hypot(loc.x - path_x[-1], loc.y - path_y[-1])
            if dist_to_goal < args.goal_tolerance:
