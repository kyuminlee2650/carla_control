r"""공개본 vs dedup950 A/B 영상의 **인지(perception) 전용** 변형본.

render_ab_quad.py 와 다른 점은 셋이다.

  1. CONTROL 패널을 뺐다. 이 영상으로 보여줄 것이 제어 입력이 아니라 무엇을 어떻게
     보았는가이기 때문이다. 그 자리를 전부 BEV 에 준다.
  2. BEV 를 눕혔다. 원본 BEV 는 xlim/ylim ±30 의 정사각형이라 가로로 넓은 자리에
     넣으면 폭의 절반이 letterbox 로 버려진다. 그래서 세로로 긴 범위(횡 ±15,
     종 -20~+40)로 그린 뒤 이미지를 시계방향 90도 돌린다. 결과는 전방이 오른쪽을
     향하는 2:1 가로 화면이고, 에이전트 필터가 이미 횡 ±15 / 종 ±30 이므로 잘려
     나가는 정보가 없다.
  3. 선을 굵히고 범례 글자를 키웠다. 발표용으로 클래스 이름이 읽혀야 한다.

나머지(카메라 6채널, 지도벡터·경로 오버레이, 충돌 테두리, 제목 바)는 그대로다.
"""
...
TICK_DT_S = 0.05
QUAD_W, CAM_H, BOT_H, BAR_H = 1400, 525, 470, 54
BEV_LEGEND_W = 300        # 범례 폭 (발표용으로 원본 250 보다 넓힘)
...
def render_bev_wide(npz, rvs):
    """rvs.render_bev 와 같은 내용을, 눕히기 좋은 세로 범위로 다시 그린다.

    원본은 정사각형(±30 x ±30)이다. 여기서는 횡 ±15 / 종 -20~+40 으로 그려 세로 2:1 을
    만들고, 마지막에 시계방향 90도 회전해 전방이 오른쪽인 가로 2:1 로 만든다. 회전은
    이미지에 하므로 plot 코드의 좌표 규약(x=횡, y=종)은 건드리지 않는다.
    """
    import io
    import matplotlib
    matplotlib.use("Agg")
    ...
    fig, axes = plt.subplots(1, 1, figsize=(3.4, 8.4))
    # 횡 ±11: 실측상 차선이 ±8 m 안에 모여 있어 ±15 로 두면 회전 후 위아래가 비었다.
    # 종 -18~+42: 전방을 넉넉히. 회전 뒤 가로 60 m x 세로 22 m 의 약 2.7:1 이 된다.
    plt.xlim(xmin=-11, xmax=11)
    plt.ylim(ymin=-18, ymax=42)

    ms, ml, mp = npz["map_scores_3d"], npz["map_labels_3d"], npz["map_pts_3d"]
    for i in range(ms.shape[0]):
        if ms[i] < rvs.MAP_CONF_TH:
            continue
        c = rvs.MAP_COLORS[int(ml[i]) % len(rvs.MAP_COLORS)]
        axes.plot(mp[i][:, 0], mp[i][:, 1], color=c, linewidth=2.2, alpha=0.95, zorder=-1)

    for box in rvs.boxes_from(npz):
        if box.name in rvs.IGNORE_LIST:
            continue
        if box.score < rvs.CONF_TH or abs(box.center[0]) > 15 or abs(box.center[1]) > 30:
            continue
        box.render(axes, view=np.eye(4), colors=("tomato", "tomato", "tomato"),
                   linewidth=1.8, box_idx=None)
        box.render_fut_trajs_grad_color(axes, linewidth=1.8, mode_idx=[0, 1, 2, 3, 4, 5],
                                        fut_ts=rvs.FUT_TS, cmap="autumn")

    for xs, ys in ([-0.9, -0.9], [-2, 2]), ([-0.9, 0.9], [2, 2]), \
                  ([0.9, 0.9], [2, -2]), ([0.9, -0.9], [-2, -2]), ([0.0, 0.0], [0.0, 2]):
        axes.plot(xs, ys, color="mediumseagreen", linewidth=2.2, alpha=0.95)

    plan = rvs.plan_traj_from(npz)
    plan = np.concatenate((np.zeros((1, plan.shape[1])), plan), axis=0)
    y = np.sin(np.linspace(0.5 * np.pi, 1.5 * np.pi, 301))
    # color_map 은 rvs 가 mmcv 에서 가져온 것이라 rvs 네임스페이스에 있다
    from mmcv.core.bbox.structures.nuscenes_box import color_map
    axes.add_collection(LineCollection(rvs.densify(plan), colors=color_map(y[:-1], "winter"),
                                       linewidths=2.6, linestyles="solid", cmap="winter"))

    axes.axes.xaxis.set_ticks([]); axes.axes.yaxis.set_ticks([]); axes.axis("off")
    ...
    # 세로(종축이 위) -> 시계방향 90도 -> 전방이 오른쪽인 가로 화면
    return cv2.rotate(img, cv2.ROTATE_90_CLOCKWISE)


def bev_legend(bev, rvs):
    """눕힌 BEV 오른쪽에 범례를 붙인다. 발표용이라 원본보다 크게 그린다."""
    from matplotlib.colors import to_rgb
    pad = BEV_LEGEND_W
    out = cv2.copyMakeBorder(bev, 0, 0, 0, pad, cv2.BORDER_CONSTANT, None, value=(255, 255, 255))
    x0, y = bev.shape[1] + 20, 40
    cv2.putText(out, "MAP VECTORS", (x0, y), cv2.FONT_HERSHEY_SIMPLEX, 0.66, INK, 2, cv2.LINE_AA)
    y += 34
    ...
    for name, bgr in rows:
        if not name:
            y += 14
            continue
        cv2.line(out, (x0, y - 6), (x0 + 34, y - 6), bgr, 6, cv2.LINE_AA)
        cv2.putText(out, name, (x0 + 46, y), cv2.FONT_HERSHEY_SIMPLEX, 0.62, INK, 2, cv2.LINE_AA)
        y += 33
    cv2.putText(out, "forward -->", (out.shape[1] - pad + 20, out.shape[0] - 18),
...
def render_half(side, idx):
    ...
    bev = bev_legend(render_bev_wide(npz, rvs), rvs)
    cv2.putText(bev, rvs.CMD_LIST[int(npz["command"])], (18, bev.shape[0] - 18),
                cv2.FONT_HERSHEY_SIMPLEX, 0.86, (0, 0, 0), 2, cv2.LINE_AA)
    bev = _letterbox(bev, QUAD_W, BOT_H, bg=(255, 255, 255))   # 전폭 -- CONTROL 패널 없음
    quad = cv2.vconcat([grid, bev])

    # title bar: which controller, where it is in time, and its final score
    bar = np.full((BAR_H, QUAD_W, 3), (32, 32, 32), np.uint8)
    cv2.putText(bar, st["label"], (16, 37), cv2.FONT_HERSHEY_SIMPLEX, 0.88,
                (255, 255, 255), 2, cv2.LINE_AA)
    tail = "" if live else "   (run ended)"
    cv2.putText(bar, f"tick {use:04d}   t = {use * TICK_DT_S:6.2f} s{tail}", (QUAD_W - 560, 36),
