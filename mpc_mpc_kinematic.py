r"""Kinematic-model lateral MPC ("mpc-kin"), alongside the existing dynamic-model "mpc"/"mpc-kf" pair
and the "vad-pid" baseline, so all of them can be driven on the same route and compared. Structurally
this file IS mpc_mpc_KF.py (LateralMPC/VyKalmanFilter/MpcLongitudinal/run_trial/main all copied from
it, untouched) with one addition: LateralMPCKinematic + the "mpc-kin" controller branch. See
mpc_mpc_KF.py's own docstring for the "mpc"/"mpc-kf"/"vad-pid" design (route spline, longitudinal
SpeedMPC, the dynamic LateralMPC QP, the VyKalmanFilter closed-loop verification, wiring order) --
none of that changed here.

"mpc-kin" (the one addition): same QP shape (condensed box/rate-constrained OSQP problem over an
Np-step preview, Nc-step control horizon, receding horizon) as LateralMPC, but the underlying vehicle
model swaps from the 2-DOF dynamic bicycle model (tire forces via Cf/Cr, state [v_y, r, e_y, e_psi])
to a REAR-AXLE-REFERENCED KINEMATIC bicycle model (no side slip, state [e_y, e_psi] only). See
LateralMPCKinematic's own docstring for the state-space derivation. Consequence: v_y and r are no
longer hidden dynamic states that need estimating -- under this model they are algebraic functions of
vx and delta (v_y == 0 by construction at the rear axle; r == vx*tan(delta)/L). "mpc-kin" therefore
runs with NEITHER CARLA's ground-truth v_y NOR a VyKalmanFilter estimate feeding x0 -- there is no v_y
slot in x0 to fill. VyKalmanFilter/"mpc-kf" are kept in this file only so "mpc-kin" can be scored
against the dynamic-model stack (and against "vad-pid") in the same run/plots, not because "mpc-kin"
itself uses the filter. hist["v_y"] (ground truth) is still logged every tick regardless of
controller, so how far the real car actually strays from this model's v_y==0 assumption can be
checked after a drive, same as it always could for "mpc"/"mpc-kf".

"vad-pid" is carried over unchanged from mpc_mpc_KF.py (_vad_waypoints_and_target + VadPidController)
so this file can score the kinematic stack against the real Bench2DriveZoo/VAD baseline too -- any
subset of "mpc"/"mpc-kf"/"mpc-kin"/"vad-pid" can be run in one command and land in the same comparison
figures.
...
Usage (Ubuntu -- verified on this lab machine):
    ...
    cd ~/carla_control
    # kinematic model alone (3 figures, same shape as mpc_mpc.py's single-controller output)
    .venv/bin/python mpc_mpc_kinematic.py --profile constant --initial-speed 5 --save-plot --controller mpc-kin
    # kinematic vs. dynamic: does dropping v_y/r estimation cost anything on this route?
    .venv/bin/python mpc_mpc_kinematic.py --profile constant --initial-speed 5 --save-plot --controller mpc mpc-kin
    # kinematic vs. its own dynamic+KF stack vs. the real VAD/Bench2Drive PID baseline
    .venv/bin/python mpc_mpc_kinematic.py --profile step --initial-speed 10 --save-plot --record \
        --controller mpc-kin mpc-kf vad-pid
    # emergency stop at t=10s, held 5s, then straight back up to --initial-speed (see --profile step)
    .venv/bin/python mpc_mpc_kinematic.py --profile step --initial-speed 10 --step-time 10 --step-size -10 \
        --step-duration 5 --save-plot --controller mpc-kin mpc vad-pid

Usage (Windows):
    cd C:\Users\mumu2\carla_control
    # kinematic model alone (3 figures, same shape as mpc_mpc.py's single-controller output)
    .venv\Scripts\python.exe mpc_mpc_kinematic.py --profile constant --initial-speed 5 --save-plot --controller mpc-kin
    # kinematic vs. dynamic: does dropping v_y/r estimation cost anything on this route?
    .venv\Scripts\python.exe mpc_mpc_kinematic.py --profile constant --initial-speed 5 --save-plot --controller mpc mpc-kin
    # kinematic vs. its own dynamic+KF stack vs. the real VAD/Bench2Drive PID baseline
    .venv\Scripts\python.exe mpc_mpc_kinematic.py --profile step --initial-speed 10 --save-plot --record --controller mpc-kin mpc-kf vad-pid
"""
...
class LateralMPCKinematic:
    r"""MPC lateral controller over a REAR-AXLE-REFERENCED kinematic bicycle model (no side slip),
    used in place of LateralMPC's dynamic 2-DOF model for the "mpc-kin" controller.

        x = [e_y, e_psi]^T,  input delta (front steer, rad),  disturbance kappa (path curvature, 1/m)

        x_dot = A(v_x) x + B(v_x) delta + E(v_x) kappa

            A(v_x) = [[0, vx], [0, 0]]   B(v_x) = [0, vx/L]^T   E(v_x) = [0, -vx]^T   (L = lf+lr)

    Derivation: the CARLA/kinematic rear-axle bicycle model has psi_dot = vx*tan(delta)/L and, by
    construction (no slip at the rear axle), v_y == 0. Substituting v_y = 0 and r = vx*tan(delta)/L
    into LateralMPC's own dynamic-model rows for e_y_dot (= v_y + vx*e_psi) and e_psi_dot (= r -
    vx*kappa), then linearizing tan(delta) ~= delta and sin(e_psi) ~= e_psi (small-angle, same
    linearization LateralMPC's own docstring already relies on for e_y_dot), gives exactly A/B/E
    above -- v_y and r are no longer independent dynamic states here, just algebraic functions of
    (vx, delta), so there is nothing left for a Kalman filter to estimate (see the module docstring).

    No output equation (no C/D, unlike LateralMPC): under this model e_y/e_psi ARE the state (C would
    be the 2x2 identity, D=0 -- nothing to add), and r collapses to a pure function of the input alone
    (r = vx*delta/L, no state dependence at all), so "track r_ref = vx*kappa" is equivalent to "track
    delta_ref = L*kappa" directly (vx cancels) -- the classic Ackermann steering feedforward. That
    collapses into a plain quadratic tracking term on U itself, U_ff = L*kappa_preview, instead of
    needing LateralMPC's Cbar/Dbar output-condensation machinery. a_y (= vx*r under v_y==0) is dropped
    entirely rather than tracked: it would just be vx*r again -- a redundant, vx^2-rescaled copy of
    the same delta-vs-feedforward term, not a genuinely distinct physical quantity the way it is in
    the dynamic model (where a_y also carries the v_y state, which this model has fixed at 0). r_dot
    is dropped for the same reason LateralMPC never tries to track it here either -- it would need
    delta_dot, which isn't representable in a one-step-ahead linear map.

    Condensing over the horizon exactly as LateralMPC does: X = Abar x0 + Bbar U + Ebar K (state
    trajectory only, no Y). Cost

        J = X' W1 X + (M_c U - U_ff)' Wr (M_c U - U_ff) + U' W2 U + (GU-Uprev)' W3 (GU-Uprev)

    reduces to the same box/rate-constrained QP shape as LateralMPC (see its own docstring for the
    G/M_c/Uprev machinery, identical here):

        min 1/2 U' H U + f' U   s.t.  -delta_max <= U <= delta_max,
                                       Uprev-ddelta_max <= GU <= Uprev+ddelta_max
        H = 2[Bbar' W1 Bbar + M_c' Wr M_c + W2 + G' W3 G]
        f = 2[Bbar' W1 (Abar x0 + Ebar K) - M_c' Wr U_ff - G' W3 Uprev]

    (e_y_ref = e_psi_ref = 0 always, so X's own reference term drops out of f entirely -- only the
    delta-vs-feedforward term U_ff contributes a reference-driven piece, same role LateralMPC's y_ref
    played.) w_r here is a steering-feedforward tracking weight, not an output weight, and defaults to
    0 -- see --kin-w-r's own help for why (delta_ff=L*kappa is a no-slip target that's measurably too
    small once real tire slip shows up at speed). The oscillation that actually matters in practice
    lives in w_ey/w_epsi vs. w_delta, not w_r -- see --kin-w-ey's help for the closed-loop numbers
    behind this class's default weights. w_ay/w_rdot have no equivalent here at all (see above) --
    there is no CLI knob for them in this class.
    """

    N_X = 2   # [e_y, e_psi]

    def __init__(self, dt, n_p, n_c, L,
                w_ey, w_epsi, w_r, w_delta, w_ddelta,
                delta_max, ddelta_max, vx_floor=VX_EPS):
        if not (0 < n_c <= n_p):
            raise ValueError(f"need 0 < n_c <= n_p, got n_c={n_c}, n_p={n_p}")
        if not (delta_max > 0 and ddelta_max > 0):
        ...
        self.M_c = LateralMPC._build_hold_matrix(n_p, n_c)
        self.G = LateralMPC._build_rate_matrix(n_c)

        self.W1 = np.diag(np.tile([w_ey, w_epsi], n_p))
        self.Wr = w_r * np.eye(n_p)          # delta-vs-feedforward tracking, one entry per Np step
        self.W2 = w_delta * np.eye(n_c)
    ...
    def _continuous(self, vx):
        vx = max(vx, self.vx_floor)
        L = self.L
        A = np.array([[0.0, vx], [0.0, 0.0]])
        B = np.array([[0.0], [vx / L]])
        E = np.array([[0.0], [-vx]])
        return A, B, E

    def _discretize(self, vx_preview):
        """Exact ZOH per previewed step, same augmented-expm identity LateralMPC uses (see its
        _discretize docstring) -- A is singular here too (e_y is a pure integrator), and unlike
        LateralMPC's dynamic model, B(vx) is NOT constant across the horizon (it scales with vx, see
        _continuous), so it has to be re-augmented and re-exponentiated every previewed step just
        like A/E already are, rather than reusing a single self.B_cont."""
        Ad, Bd, Ed = [], [], []
        n = self.N_X
        for vx in vx_preview:
            A, B, E = self._continuous(vx)
            aug = np.zeros((n + 2, n + 2))
            aug[:n, :n] = A
            aug[:n, n:n + 1] = B
            aug[:n, n + 1:n + 2] = E
    ...
    def _condense(self, Ad, Bd, Ed):
        """State-only condensation, no Y = CX + DU layer (see class docstring for why LateralMPC's
        Cbar/Dbar machinery isn't needed here)."""
        n_p, n_x = self.n_p, self.N_X
        A_bar = np.zeros((n_p * n_x, n_x))
        B_full = np.zeros((n_p * n_x, n_p))
        E_bar = np.zeros((n_p * n_x, n_p))
        ...
        B_bar = B_full @ self.M_c
        return A_bar, B_bar, E_bar

    def solve(self, x0, vx_preview, kappa_preview):
        """x0 = [e_y, e_psi] (vehicle-minus-path sign convention, same as LateralMPC -- but no v_y/r
        slots: this model has neither, see class docstring). vx_preview/kappa_preview: length-Np
        previews. Returns delta_cmd (rad, clipped to +-delta_max)."""
        vx_preview = np.clip(np.asarray(vx_preview, dtype=float), self.vx_floor, None)
        kappa_preview = np.asarray(kappa_preview, dtype=float)
        x0 = np.asarray(x0, dtype=float).reshape(-1, 1)

        Ad, Bd, Ed = self._discretize(vx_preview)
        A_bar, B_bar, E_bar = self._condense(Ad, Bd, Ed)

        K = kappa_preview.reshape(-1, 1)
        U_ff = (self.L * kappa_preview).reshape(-1, 1)   # Ackermann feedforward, see class docstring

        U_prev = np.zeros((self.n_c, 1))
        U_prev[0, 0] = self.last_solution[0]

        H = 2.0 * (B_bar.T @ self.W1 @ B_bar + self.M_c.T @ self.Wr @ self.M_c
                  + self.W2 + self._GtW3G)
        H = 0.5 * (H + H.T)
        f = 2.0 * (B_bar.T @ self.W1 @ (A_bar @ x0 + E_bar @ K)
                  - self.M_c.T @ self.Wr @ U_ff - self.G.T @ self.W3 @ U_prev)
...
def main():
    ...
    lat.add_argument("--w-ey", type=float, default=10.0,
                     help="cross-track error weight, 'mpc'/'mpc-kf' only -- see --kin-w-ey for 'mpc-kin'")
    lat.add_argument("--w-epsi", type=float, default=10.0,
    ...
    lat.add_argument("--w-delta", type=float, default=1,
    ...
    lat.add_argument("--delta-max-deg", type=float, default=30.0,
                     help="hard steer-magnitude limit (deg, bicycle-model wheel angle), shared by "
                          ...
                          "over the range they were calibrated/derived for")
    lat.add_argument("--ddelta-max-deg", type=float, default=70.0,
                     help="hard steer-rate limit (deg/s), shared by 'mpc'/'mpc-kf'/'mpc-kin'")
    ...
    kin.add_argument("--kin-w-ey", type=float, default=3.0,
                     help="cross-track error weight -- kept far below the dynamic model's --w-ey (10) "
                          "on purpose. A 10 m/s closed-loop check on this route (Town10HD_Opt, same "
                          "route --spawn-x/-y drives) found w_ey>=6 (with w_epsi scaled alongside it) "
                          "throws the loop into steer oscillation: yaw-rate peaks ~90-98 deg/s, a_y "
                          "peaks 12-14 m/s^2, cross-track RMSE ~0.30 m. Isolating the cause (holding "
                          "--kin-w-r/--kin-w-ddelta at their old values and only lowering this + "
                          "--kin-w-epsi) showed THIS pair, not --kin-w-r or --kin-w-ddelta, is what "
                          "actually fixes it -- w_ey/w_epsi too high relative to --kin-w-delta makes "
                          "the QP demand corrections sized for a no-slip vehicle; the real (slipping) "
                          "car can't deliver them in one step, the error doesn't close, and the next "
                          "tick's correction gets even larger. w_ey=w_epsi=3 stayed well-behaved at "
                          "10 m/s (cross-track RMSE 0.14 m, yaw-rate peak ~65 deg/s) at the cost of a "
                          "little at 5 m/s (RMSE 0.18 m vs 0.11 m at the old higher gains, still "
                          "sub-half-meter peak) -- tuned for the faster/tighter-corner case since that's "
                          "this route's harder regime (see --initial-speed's own note)")
    kin.add_argument("--kin-w-epsi", type=float, default=3.0, help="heading error weight -- see --kin-w-ey")
    ...
    parser.add_argument("--controller", nargs="+", default=["mpc-kin"],
                        choices=("mpc", "mpc-kf", "mpc-kin", "vad-pid"),
                        help="which controller(s) to run this route with. One controller (default): "
                             ...
                             "'mpc' and 'mpc-kf' run the identical dynamic-model LateralMPC/"
                             "MpcLongitudinal pair -- 'mpc' feeds x0 CARLA's own ground-truth v_y, "
                             "'mpc-kf' feeds it VyKalmanFilter's online estimate instead "
                             "(kalman_filter.py). 'mpc-kin' runs LateralMPCKinematic instead of "
                             "LateralMPC -- a rear-axle kinematic bicycle model (state [e_y, e_psi] "
                             "only, no v_y/r, no Kalman filter involved at all) -- against the same "
                             "MpcLongitudinal; see LateralMPCKinematic's own docstring for the "
                             "state-space derivation and the module docstring for why this file exists. "
    ...
    controller_labels = {"mpc": "MPC", "mpc-kf": "MPC-KF", "mpc-kin": "MPC-KIN", "vad-pid": "VAD-PID"}
